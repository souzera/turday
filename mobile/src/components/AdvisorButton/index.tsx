...
export default function AdvisorButton(props: AdvisorButtonProps) {
    ...
    const validateAvatar = (avatar: string) => {
      return avatar.startsWith('/uploads') ? `${options.API_URL}${avatar.substring(1)}` : avatar;
    };

    props.avatar && console.log(validateAvatar(props.avatar));

    const handleChat = () => {
        console.log(`Chat to advisor ${props.nome}: Send message to ${props.contato}`);
    }
  ...
  return (
    <View style={styles.container}>
      <View style={styles.panel}>
        <View>
          {props.avatar ? (
            <Image source={{ uri: validateAvatar(props.avatar) }} style={styles.avatar} />
          ) : (
        ...
        <View>
          <TouchableOpacity
            onPress={handleChat}
          >
            <Ionicons name="chatbubble-ellipses" size={32} color={THEME.COLORS.DARKGRAY} />
          </TouchableOpacity>
        </View>
      </View>
    </View>
